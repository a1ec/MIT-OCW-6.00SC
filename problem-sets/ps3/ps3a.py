...
def update_hand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    # make copy of hand so no side effect
    h = dict(hand)
    for c in word:
        # decrement the value of times the letter occurs in the word from key
        h[c] -= 1
...
def generate_hand_from(word):
    d = dict()
    # generate a dict based on word
    for c in word: d[c] = word.count(c)
    return d
#
